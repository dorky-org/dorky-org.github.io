...
**证明**：在单位圆上取两点 $A(\cos\alpha,\sin\alpha)$、$B(\cos\beta,\sin\beta)$，用两点间距离公式：
...
两个 $AB^2$ 的表达式相等，得 $2-2(\cos\alpha\cos\beta+\sin\alpha\sin\beta)=2-2\cos(\alpha-\beta)$，即证。由这一条推出其余三条：
...
**正切的和差角公式**由 $\tan=\dfrac{\sin}{\cos}$ 直接推出：分子分母同除以 $\cos\alpha\cos\beta$，

$\tan(\alpha+\beta)=\dfrac{\tan\alpha+\tan\beta}{1-\tan\alpha\tan\beta}$，$\tan(\alpha-\beta)=\dfrac{\tan\alpha-\tan\beta}{1+\tan\alpha\tan\beta}$（把 $\beta$ 换成 $-\beta$ 直接得到）
...
$a\sin x+b\cos x=\sqrt{a^2+b^2}\sin(x+\varphi)$，其中 $\cos\varphi=\dfrac{a}{\sqrt{a^2+b^2}}$，$\sin\varphi=\dfrac{b}{\sqrt{a^2+b^2}}$（$\varphi$ 由这两个式子共同确定所在象限）

**证明**：把 $\sqrt{a^2+b^2}$ 提出来：
