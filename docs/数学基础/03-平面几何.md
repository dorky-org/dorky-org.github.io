...
**外角定理**：三角形的一个外角，等于与它不相邻的两个内角之和。图左中 $\angle ACD = \angle A + \angle B$。

由内角和立刻可得：$\angle ACD = 180^{\circ}-\angle ACB = \angle A + \angle B$。这条在上一章圆周角定理的证明里是关键一步。
...
**全等**是形状和大小都相同，**相似**只要求形状相同、大小可以不同。相似是这一节的重点，因为相机成像就是一个相似变换。
...
它是相似三角形的来源——$DE \parallel BC$ 时 $\triangle ADE \sim \triangle ABC$。反过来也成立，比例相等可以反推平行。
...
过点 $C$ 作 $CE \parallel AD$，与 $BA$ 的延长线交于点 $E$（图右）。
...
**另一种推法**（用第四节的面积公式，两行）：$\triangle ABD$ 与 $\triangle ACD$ 共用从 $A$ 出发的高，面积比等于底之比 $BD:DC$；同时按 $S=\frac{1}{2}ab\sin C$，两者面积比又等于 $AB \cdot AD \sin\angle BAD$ 与 $AC \cdot AD \sin\angle DAC$ 之比，因两角相等，化简即得 $AB:AC$。两式相等即证。

**外角平分线**同样成立，只是 $D$ 落在 $BC$ 的延长线上，结论写成 $\dfrac{AB}{AC} = \dfrac{BD}{DC}$ 不变。
...
**证明**（只用相似，不用三角函数）：设两线交于 $P$，取 $l_1$ 上一点 $A$、$l_2$ 上一点 $B$（在 $P$ 两侧，如图），分别向过 $P$ 的水平参照线作垂线，垂足是 $C$、$D$。
...
第三步，代入斜率。设 $C$、$D$ 在参照线上分别位于 $P$ 的右侧、左侧（如图），则

$k_1 = \dfrac{CA}{PC}$，$k_2 = -\dfrac{DB}{PD}$

负号是因为 $B$ 相对 $P$ 在 $x$ 减小的方向，斜率必须为负（$\theta$ 是锐角，$k_2$ 的大小是 $DB/PD$，符号由方向决定）。两式相乘：
...
**在具身智能里的用途**：法线方向与切线方向垂直（$k_{\text{法}} = -1/k_{\text{切}}$），用在光线反射、梯度下降的方向与等高线垂直、SDF 梯度与表面垂直等场景。三维情形对应向量点积为零 $\mathbf{u}\cdot\mathbf{v}=0$，这条二维斜率公式是它的坐标特例。

### 图示：针孔相机成像

![针孔相机的相似三角形](assets/针孔相机的相似三角形.svg)

物体高 $X$，距光心 $Z$；成像平面距光心 $f$，像高 $x$，倒立。

光线从物体顶端穿过光心射到成像平面，在光心两侧形成一对**对顶的相似三角形**（一组对顶角相等，两组直角相等，由 AA 判定相似）。于是

$\dfrac{x}{f} = \dfrac{X}{Z}$

这就是针孔相机模型的全部内容。变形得 $x = f\dfrac{X}{Z}$：像高与物高成正比，与距离成反比。单目相机只能测出 $\dfrac{X}{Z}$ 这个比值，无法单独定出 $X$ 或 $Z$——这就是**单目尺度不确定性**的几何来源。
...
推到 $n$ 维就是向量的二范数 $\lVert \mathbf{v} \rVert_2 = \sqrt{\sum v_i^2}$。**勾股定理是欧氏距离的唯一来源**，后面所有"距离"的定义都以它为原型。
...
**证明**要按圆心 $O$ 相对于 $\angle BAC$ 的位置分三种情况，图上三栏依次对应。核心工具只有两个：**半径相等构成等腰三角形**，以及**三角形的外角等于不相邻两内角之和**。

**情况一，圆心在角的一边上**（图左，$AB$ 恰好是直径）。

连 $OC$。因为 $OA = OC$（同圆半径），$\triangle OAC$ 是等腰三角形，所以
...
作直径 $AD$，把 $\angle BAC$ 拆成 $\angle BAD$ 与 $\angle DAC$。这两个角各自都落回情况一，于是
...
同样作直径 $AD$，此时 $\angle BAC$ 是两个角的**差**而不是和：
...
因为它们都等于同一个圆心角的一半。图左中 $P$、$Q$ 是优弧上任意两点，$\angle BPC = \angle BQC$，与点在优弧上怎么滑动无关。

这条的逆命题给出**四点共圆的判定**：若 $P$、$Q$ 在直线 $BC$ 同侧且 $\angle BPC = \angle BQC$，则 $B$、$C$、$P$、$Q$ 四点共圆。

**推论二：半圆（直径）所对的圆周角是直角。**

直径对应的圆心角是平角 $180^{\circ}$，一半即 $90^{\circ}$。
...
**证明**：垂直平分线的本质是"到线段两端距离相等的点的集合"。设 $BC$ 与 $AB$ 的垂直平分线交于 $O$，则

- $O$ 在 $BC$ 的垂直平分线上 $\Rightarrow OB = OC$
- $O$ 在 $AB$ 的垂直平分线上 $\Rightarrow OA = OB$

于是 $OA = OB = OC$。由 $OA = OC$ 反推，$O$ 也必在 $AC$ 的垂直平分线上，三线共点。以 $O$ 为圆心、$OA$ 为半径的圆同时过三个顶点。
...
**证明**：角平分线的本质是"到角两边距离相等的点的集合"。设 $\angle A$ 与 $\angle B$ 的平分线交于 $I$，则

- $I$ 在 $\angle A$ 的平分线上 $\Rightarrow$ $I$ 到 $AB$ 与 $AC$ 的距离相等
- $I$ 在 $\angle B$ 的平分线上 $\Rightarrow$ $I$ 到 $AB$ 与 $BC$ 的距离相等

于是 $I$ 到三条边的距离全部相等。由"到 $AC$、$BC$ 距离相等"反推，$I$ 也在 $\angle C$ 的平分线上，三线共点。以 $I$ 为圆心、这个公共距离为半径的圆与三边都相切（半径垂直于边，正是切线判定）。

内心永远在三角形内部，这一点和外心不同。

内切圆半径由面积得出：把三角形按内心切成三块，每块以一条边为底、以 $r$ 为高，于是 $S = \dfrac{1}{2}r(a+b+c) = rp$，其中 $p$ 是半周长。所以
...
上面讲了外心和内心，把四个心放在一起对照，其中**重心**在具身智能里用得最多。
...
第二条把重心和"质心"直接连起来：均匀薄片三角形的质心就是重心；推广到 $n$ 个点，质心就是 $\dfrac{1}{n}\sum P_i$，这正是点云求中心的做法。
...
推广到 $n$ 个点，质心就是 $\dfrac{1}{n}\sum P_i$，这正是点云求中心的做法。
...
图中绿色虚线是三条高，垂足处的直角标记表示它们确实垂直于对边；紫色线是欧拉线，$O$、$G$、$H$ 依次排开。下面把证明拆成能一步步核对的小步骤。
...
这只是三个坐标相加，此刻还不知道它是谁，先验证它满足"垂心"的定义——从每个顶点出发的高都经过它。只需验证 $AH \perp BC$（其余两条同理，换个字母重复一遍就行）。

**先说清楚"两点间的向量"怎么用位置向量表示**：对任意两点 $P$、$Q$，从 $P$ 指向 $Q$ 的向量 $\overrightarrow{PQ}$ 等于 $Q$ 的位置向量减去 $P$ 的位置向量，即 $\overrightarrow{PQ}=\mathbf q-\mathbf p$。这是向量加法的三角形法则的直接推论：从 $O$ 走到 $P$ 再走到 $Q$，等于直接从 $O$ 走到 $Q$，即 $\mathbf p+\overrightarrow{PQ}=\mathbf q$，移项就得到 $\overrightarrow{PQ}=\mathbf q-\mathbf p$。
...
判断两个几何体的相对位置，只需要比较**距离**和**半径**。这是碰撞检测最基础的一层。
...
写成一个量就是 $d - r$：负数在内、零在边界、正数在外。**这正是圆的符号距离函数（SDF）**，学习路径第 88 行的 SDF 就是把它推广到任意形状。
...
判别时用点到直线距离公式算 $d$，不必联立方程。射线与球求交（光线追踪、激光测距仿真）就是这条的三维版。
...
第一行的否定就是最常用的碰撞判据：**两个圆盘相碰 $\iff d \le r_1+r_2$**。给每个物体套一个包围圆（三维是包围球），先用这一个不等式做粗筛，通过了再做精确检测——这是所有碰撞检测系统的第一层。
...
**这就是弧度制存在的理由**：它让弧长公式退化成一个乘法。第 9 行三角函数、以及后面所有涉及角速度的地方（$v = \omega r$ 就是 $l=\theta r$ 对时间求导）都默认用弧度。
...
**证明**：连 $AC$、$DB$。$\angle PAC$ 与 $\angle PDB$ 是同弧 $\overset{\frown}{BC}$ 所对的圆周角（分别是 $\angle CAB$、$\angle CDB$ 的一部分/延伸），由圆周角定理相等；$\angle APC$ 与 $\angle DPB$ 是对顶角，也相等。于是
...
**证明**：连 $AC$、$DB$。$\angle PAC$ 与 $\angle PDB$ 分别是 $\angle A$ 处、$\angle D$ 处的角，$A$、$B$、$C$、$D$ 四点共圆，$\angle PAC$ 与 $\angle BDC$（即 $\angle PDB$）同样是同弧 $\overset{\frown}{BC}$ 所对的圆周角，相等；$\angle P$ 是 $\triangle PAC$ 与 $\triangle PDB$ 的公共角。于是同样由 AA 得

$\triangle PAC \sim \triangle PDB$

$\dfrac{PA}{PD} = \dfrac{PC}{PB} \;\Rightarrow\; PA \cdot PB = PC \cdot PD$

**这两种情形用的是同一个相似三角形结构**，只是 $P$ 在圆内还是圆外决定了 $\angle APC$ 与 $\angle DPB$ 是对顶角还是公共角。

**情形三，切割线定理**（$P$ 在圆外，一条线是切线）：切线切于 $T$，割线交圆于 $A$、$B$（$A$ 是近点），则

$PT^2 = PA \cdot PB = d^2-r^2$

**证明**要先补一条引理——**切线弦角定理**：切线与弦的夹角，等于这条弦在弦的另一侧所对的圆周角。即图中 $\angle PTA = \angle TBA$。
...
这是激光点云里拟合圆柱、拟合弯道的基础：随机取三点算出一个圆，统计有多少点落在容差范围内，重复若干次取最好的，就是 RANSAC 拟合圆。所需的最少采样点数 $3$，正是由"三点定圆"决定的。
...
**证明**（坐标公式）：设 $\mathbf{u}=(x_2-x_1,\ y_2-y_1)$、$\mathbf{v}=(x_3-x_1,\ y_3-y_1)$，即 $A$ 出发的两条边 $AB$、$AC$。

![面积与叉积](assets/面积与叉积.svg)

平移不改变面积，把 $A$ 移到原点，问题变成：向量 $\mathbf{u}$、$\mathbf{v}$ 张成的三角形面积是多少。
...
几何上看，$\lvert u_1v_2-u_2v_1\rvert$ 是 $\mathbf{u}$、$\mathbf{v}$ 张成的**平行四边形**的面积（图中黄色三角形与蓝色三角形拼成的平行四边形），三角形恰好是它的一半——这也是"叉乘的模等于两向量张成平行四边形面积"这条性质在二维的由来。

绝对值内部那一坨 $u_1v_2-u_2v_1$，就是向量 $\mathbf{u}=(x_2-x_1,\ y_2-y_1)$ 与 $\mathbf{v}=(x_3-x_1,\ y_3-y_1)$ 的**二维叉乘**。它有三个用处：
