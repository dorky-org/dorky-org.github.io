...
**外角定理**：三角形的一个外角，等于与它不相邻的两个内角之和。图左中 $\angle ACD = \angle A + \angle B$（由内角和直接得到，证明见后面）。这条在上一章圆周角定理的证明里是关键一步。
...
$DE \parallel BC$ 且 $DE = \dfrac{1}{2}BC$（证明见后面）
...
$\angle A + \angle C = 180^{\circ}$，$\angle B + \angle D = 180^{\circ}$（证明见后面）
...
**垂直于弦的直径，平分这条弦，并且平分弦所对的两条弧。**（证明见后面）
...
**切线性质：圆的切线垂直于过切点的半径。**（反证法，证明见后面）

**切线判定**：过半径外端且垂直于这条半径的直线是圆的切线。

**切线长定理**：从圆外一点 $Q$ 引圆的两条切线，两条切线长相等，即 $QP_1 = QP_2$；且 $OQ$ 平分 $\angle P_1QP_2$。（证明见后面）
...
由内角和立刻可得：$\angle ACD = 180^{\circ}-\angle ACB = \angle A + \angle B$。

### 中位线定理的证明

用相似：$AD:AB = AE:AC = 1:2$ 且夹角 $\angle A$ 公共，由 SAS 得 $\triangle ADE \sim \triangle ABC$，相似比 $1:2$，于是对应边成比例、对应角相等（同位角相等推出平行）。
...
$\angle A$ 与 $\angle C$ 分别对着 $\overset{\frown}{BCD}$ 和 $\overset{\frown}{BAD}$ 这两条弧，它们合起来是整个圆周 $360^{\circ}$。由圆周角定理，两角之和等于这两条弧对应圆心角之和的一半，即 $\dfrac{360^{\circ}}{2}=180^{\circ}$。

### 垂径定理的证明

连 $OA$、$OB$。$OA = OB$（半径），故 $\triangle OAB$ 等腰。等腰三角形三线合一——顶角平分线、底边中线、底边高互相重合。已知 $OD \perp AB$ 于 $M$，即 $OM$ 是底边上的高，所以它同时是中线，$AM = MB$；也是顶角平分线，$\angle AOD = \angle BOD$，所以两条弧也相等。

### 切线性质的证明

反证法：设切线 $l$ 与圆切于 $P$，若 $OP$ 不垂直于 $l$，作 $OH \perp l$ 于 $H$，则 $OH \lt OP = r$。这说明圆心到直线的距离小于半径，直线必与圆有两个交点，与"切线只有一个公共点"矛盾。故 $OP \perp l$。

### 切线长定理的证明

$OP_1 = OP_2 = r$，$\angle OP_1Q = \angle OP_2Q = 90^{\circ}$，$OQ$ 公共，由 HL 判定 $\triangle OP_1Q \cong \triangle OP_2Q$，于是对应边 $QP_1 = QP_2$、对应角 $\angle P_1QO = \angle P_2QO$。

### 外心是三边垂直平分线交点的证明

垂直平分线的本质是"到线段两端距离相等的点的集合"。设 $BC$ 与 $AB$ 的垂直平分线交于 $O$，则
...
角平分线的本质是"到角两边距离相等的点的集合"。设 $\angle A$ 与 $\angle B$ 的平分线交于 $I$，则
...
把三角形按内心切成三块，每块以一条边为底、以 $r$ 为高，于是 $S = \dfrac{1}{2}r(a+b+c) = rp$，其中 $p$ 是半周长，解得 $r=\dfrac{S}{p}$。
