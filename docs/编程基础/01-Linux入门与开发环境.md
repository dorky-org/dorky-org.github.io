...
| 软件从哪来 | apt、pip、源码编译三条路各装到哪 | 第 5 行的编译心智模型、第 13 行虚拟环境、一切"版本冲突" |
| sudo 的边界 | 什么时候该用、什么时候绝不能用 | 别把系统搞坏，也别把 Python 环境搞坏 |
| 一组基本命令 | 十几条，够用就行 | 从明天起每天都在敲 |

这一篇对应[学习路径](学习路径.md)第 1 行，12 小时。**最容易被低估的是第一节**：Ubuntu 版本选错，后面装 ROS 2 时会发现官方根本没有对应的包，只能重装系统。这件事排在最前面不是凑数。
...
- **`pip install` 前面永远不要加 `sudo`。** 加了会把包装进系统 Python，和 apt 装的包互相覆盖，最后两边都坏。Ubuntu 23.04 之后系统会直接拦住你，报 `externally-managed-environment`——**那不是错误，那是保护**。正确做法在第 13 行：用虚拟环境
...
出问题时先问一句：**我现在用的到底是哪一份？** 第 5 行讲的 `PATH`、`LD_LIBRARY_PATH` 就是回答这个问题的工具。
...
- **[Missing Semester 中文版](https://missing-semester-cn.github.io/)**：这门课从 shell 讲起，**不讲装系统、不讲目录结构、不讲 apt**。它的第 1、2 讲对应本路径第 2 行（命令行日常），这一篇之后再看正好
