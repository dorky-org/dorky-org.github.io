...
| :---: | --- |
| `*Counter` | 统计每个元素出现的次数，自带 `most_common()`<br/> 取出现最多的几个 |
| `defaultdict` | 键不存在时自动生成默认值，不用先判断再赋值，避免 `KeyError` |
| `deque` | 两端增删都很快，配合 `maxlen`<br/> 做固定长度的滑动窗口（传感器数据缓冲） |
| `namedtuple` | 给元组的每一项起名字，用属性名代替裸下标取值，可读性好 |
| `OrderedDict` | 记住插入顺序的字典（Python 3.7+ 普通 `dict`<br/> 已有此特性，现在用得少） |
| `ChainMap` | 把多个字典串成一个来查，常用于"默认配置 + 用户配置"这种多层覆盖 |
